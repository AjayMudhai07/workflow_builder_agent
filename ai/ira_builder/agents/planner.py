...
from ai.ira_builder.tools.validation_tools import (
...
"""
    ...
    def set_file_analysis(self, file_analysis_results: Dict[str, Any]):
        """
        Store file analysis results in memory.

        Args:
            file_analysis_results: Dictionary containing:
                - files: List of file analysis results with column descriptions
                - dataset_description: Overall dataset description
                - total_files, total_columns, total_rows
        """
        self.file_analysis_results = file_analysis_results
        self.dataset_description = file_analysis_results.get("dataset_description")
        logger.info("Stored file analysis results in memory")
    ...
    async def invoking(self, messages, **kwargs):
        ...
        logger.info(f"  - Has file analysis: {bool(self.file_analysis_results)}")

        if self.csv_analysis or self.workflow_context or self.file_analysis_results:
            ...
            if self.file_analysis_results:
                context_parts.append("\n**File Analysis (Column Descriptions - ALREADY ANALYZED):**\n")
                for file_info in self.file_analysis_results.get("files", []):
                    file_name = file_info.get("file_name", "Unknown")
                    file_desc = file_info.get("file_description", "No description")
                    context_parts.append(f"\n**{file_name}:**")
                    context_parts.append(f"Description: {file_desc}")
                    context_parts.append("Columns:")
                    for col_desc in file_info.get("column_descriptions", []):
                        col_name = col_desc.get("name", "Unknown")
                        col_description = col_desc.get("description", "No description")
                        context_parts.append(f"  - {col_name}: {col_description}")
                context_parts.append("\n**NOTE:** These column and file descriptions were generated by AI analysis and are available in your memory.")
...
class PlannerAgent:
    ...
    def __init__(
        ...
        if chat_client is None:
            ...
            if config.openai_api_key:
                # Explicitly set environment variable from config to ensure .env takes precedence
                import os
        ...
        config = get_config()
        is_groq = config.llm_provider.lower() == "groq"

        # For Groq, disable tools due to function calling compatibility issues
        # We rely on pre-analysis instead
        if is_groq:
            logger.info("Using Groq provider - disabling tools (will use pre-analysis instead)")
            agent_tools = []
        else:
            agent_tools = [
                analyze_csv_structure,
    ...
    async def initialize_workflow(
        ...
        try:
            ...
            # Check if using Groq (must pre-analyze since tools are disabled)
            config = get_config()
            is_groq = config.llm_provider.lower() == "groq"
            ...
            try:
                ...
                # For Groq, pre-analysis is mandatory since tools are disabled
                if is_groq:
                    raise AgentException(
                        f"Failed to analyze CSV files (required for Groq): {str(csv_error)}"
                    )
                # For OpenAI, continue anyway - agent will analyze during conversation

            # Create initial context prompt
            # For Groq or if CSV was pre-analyzed, tell agent it's already done
            if is_groq or self.csv_memory.csv_analysis:
                context_prompt = f"""
...
**IMPORTANT:** The CSV files have ALREADY been analyzed and the structure is available in your memory (injected as context). You do NOT need to call any CSV analysis tools.
...
def create_planner_agent(
    ...
    logger.info(f"Creating Planner Agent (provider={provider}, model={model or 'default'}, azure={use_azure})")
